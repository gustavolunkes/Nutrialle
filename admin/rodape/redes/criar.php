...
$page_title     = 'Nova Rede Social';
$current_module = 'rodape';
$current_page   = 'rodape-rede-criar';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$rede || !$url) { $error = 'Rede e URL são obrigatórios.'; }
    else {
        try {
            $db = getDB();
            $db->prepare("INSERT INTO rodape_redes_sociais (rede,url,ordem,ativo) VALUES (?,?,?,?)")
               ->execute([$rede, $url, $ordem, $ativo]);
            $_SESSION['success'] = 'Rede social adicionada!';
...
<div class="wrap">
    ...
    <div class="card">
        <div class="card-title">Escolha a rede e configure</div>
        <form method="POST">
            <div class="form-grid">
                <div class="fg full">
                    <label>Rede Social <span class="req">*</span></label>
                    <div class="rede-picker">
                        <?php foreach ($redesOpcoes as $r): $sel = (($_POST['rede']??'')===$r); ?>
                ...
                <div class="fg full">
                    ...
                    <input type="url" name="url" class="fc" required value="<?= htmlspecialchars($_POST['url']??'') ?>" placeholder="https://facebook.com/suaempresa">
            ...
            <div class="form-actions">
                <button type="submit" class="btn btn-dark">✅ Adicionar Rede</button>
