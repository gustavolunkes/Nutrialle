...
$menus = $db->query("SELECT id, name, slug FROM menus ORDER BY name ASC")->fetchAll();
$menus_map = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!$slug) { $error = 'O slug é obrigatório.'; }
        ...
        try {
            $chk = $db->prepare("SELECT id FROM categorias WHERE slug = ? AND menu_id = ? AND id != ?");
            $chk->execute([$slug, $menu_id, $categoria_id]);
            if ($chk->fetch()) {
                $error = 'Slug já em uso neste menu.';
            } else {
                $db->prepare("UPDATE categorias SET menu_id=?,name=?,slug=?,url=?,target=?,description=?,order_position=?,active=? WHERE id=?")
                   ->execute([$menu_id, $name, $slug, $url ?: null, $target, $description, $order_position, $active, $categoria_id]);
                $_SESSION['success'] = "Categoria <strong>$name</strong> atualizada com sucesso!";
                header('Location: ' . BASE_URL . '/admin/menus/index.php');
                exit;
            }
        } catch (Exception $e) { $error = 'Erro: ' . $e->getMessage(); }
    }
    // atualiza objeto local para re-render
    $categoria = array_merge($categoria, compact('menu_id','name','slug','url','target','description','order_position','active'));
}
...
<div class="pg-wrap">
    ...
    <div class="breadcrumb">
        <div class="bc-item bc-menu">
            ...
            <span id="bc-menu-name"><?= htmlspecialchars($menus_map[$categoria['menu_id']]['name'] ?? '—') ?></span>
        </div>
        <span class="bc-sep">›</span>
        <div class="bc-item bc-cat">
            <span class="label">🏷️ Categoria</span>
            <span id="bc-cat-name"><?= htmlspecialchars($categoria['name']) ?></span>
        </div>
    </div>
    ...
    <div class="meta-bar">
        ...
        <span>📅 Criado em: <?= date('d/m/Y H:i', strtotime($categoria['created_at'])) ?></span>
        <span>🔄 Atualizado: <?= date('d/m/Y H:i', strtotime($categoria['updated_at'])) ?></span>
    </div>
    ...
    <div class="card">
        ...
        <div class="form-grid">
            <div class="fg full">
                ...
                <select id="sel-menu" class="fc">
                    <option value="">— Selecione o menu pai —</option>
                    <?php foreach ($menus as $m): ?>
                        <option value="<?= $m['id'] ?>" data-name="<?= htmlspecialchars($m['name']) ?>" <?= $categoria['menu_id'] == $m['id'] ? 'selected' : '' ?>>
                            📋 <?= htmlspecialchars($m['name']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <span class="hint">Trocar o menu não afeta as subcategorias vinculadas a esta categoria</span>
            </div>
        </div>

        <p class="section-title">Dados da Categoria</p>
        <form method="POST" id="form-cat">
            <input type="hidden" name="menu_id" id="hid-menu" value="<?= $categoria['menu_id'] ?>">
            <div class="form-grid">
                <div class="fg">
                    <label>Nome <span class="req">*</span></label>
                    <input type="text" name="name" id="inp-name" class="fc" value="<?= htmlspecialchars($categoria['name']) ?>" required placeholder="Nome da categoria">
                </div>
                <div class="fg">
                    <label>Slug (URL) <span class="req">*</span></label>
                    <input type="text" name="slug" id="inp-slug" class="fc" value="<?= htmlspecialchars($categoria['slug']) ?>" required placeholder="nome-da-categoria">
                    <div class="slug-preview">🔗 /<b id="slug-val"><?= htmlspecialchars($categoria['slug']) ?></b></div>
                </div>
                <div class="fg">
                    <label>URL Externa</label>
                    <input type="url" name="url" class="fc" value="<?= htmlspecialchars($categoria['url'] ?? '') ?>" placeholder="https://...">
                    <span class="hint">Deixe vazio para vincular uma página interna</span>
                </div>
                <div class="fg">
                    <label>Abrir link em</label>
                    <select name="target" class="fc">
                        <option value="_self" <?= ($categoria['target'] ?? '_self') == '_self' ? 'selected' : '' ?>>📄 Mesma aba</option>
                        <option value="_blank" <?= ($categoria['target'] ?? '_self') == '_blank' ? 'selected' : '' ?>>🔗 Nova aba</option>
                    </select>
                </div>
                <div class="fg">
                    <label>Ordem de exibição</label>
                    <input type="number" name="order_position" class="fc" min="0" value="<?= $categoria['order_position'] ?>">
                    <span class="hint">Menor número aparece primeiro</span>
                </div>
                <div class="fg full">
                    <label>Descrição</label>
                    <textarea name="description" class="fc" placeholder="Descrição opcional para SEO..."><?= htmlspecialchars($categoria['description'] ?? '') ?></textarea>
                </div>
                <div class="fg full">
                    <div class="tog-wrap">
                        <label class="tog"><input type="checkbox" name="active" <?= $categoria['active'] ? 'checked' : '' ?>><span class="tog-s"></span></label>
...
inpName.addEventListener('input', () => {
    bcCat.textContent = inpName.value || '—';
});
