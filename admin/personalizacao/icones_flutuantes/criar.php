...
$page_title     = 'Novo Ícone Flutuante';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        try {
            $db->prepare("
                INSERT INTO icones_flutuantes (label, icone, url, target, cor_fundo, cor_icone, posicao, ordem, ativo)
                VALUES (:label, :icone, :url, :target, :cor_fundo, :cor_icone, :posicao, :ordem, :ativo)
            ")->execute($fields);
            $_SESSION['success'] = 'Ícone criado com sucesso!';
...
// Valores para repopular form
$v = [
    'label'     => $_POST['label']    ?? '',
    'icone'     => $_POST['icone']    ?? '💬',
    'url'       => $_POST['url']      ?? '',
    'target'    => $_POST['target']   ?? '_blank',
    'cor_fundo' => $_POST['cor_fundo']?? '#25d366',
    'cor_icone' => $_POST['cor_icone']?? '#ffffff',
    'posicao'   => $_POST['posicao']  ?? 'bottom-right',
    'ordem'     => $_POST['ordem']    ?? 0,
    'ativo'     => $_POST['ativo']    ?? 1,
];
...
/* Preview bolinha */
.preview-ball{width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:24px;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.25);transition:transform .2s;flex-shrink:0}
.preview-ball:hover{transform:scale(1.08)}
.preview-section{display:flex;align-items:center;gap:16px;padding:20px;background:#f8fafc;border-radius:10px;margin-top:0}
...
<div class="wrap">
    ...
    <form method="POST">
            ...
            <div class="card-body">
                <div class="form-grid">
                    <div class="fg">
                        ...
                        <input type="text" name="label" class="fc" required
                               value="<?= hce($v['label']) ?>" placeholder="Ex: WhatsApp">
                        <span class="hint">Texto exibido ao passar o mouse.</span>
                    </div>
                    <div class="fg">
                        <label>URL de Destino *</label>
                        <input type="url" name="url" class="fc" required
                               value="<?= hce($v['url']) ?>" placeholder="https://wa.me/55...">
                    </div>
                    <div class="fg">
                    ...
                    <div class="fg">
                        ...
                        <span class="hint">Menor número aparece primeiro (de baixo p/ cima).</span>
                    </div>
            ...
            <div class="card-body">
                <div class="fg">
                    ...
                    <input type="text" name="icone" id="inputIcone" class="fc" required
                           ...
                           placeholder="Cole um emoji ou caractere"
                           oninput="document.getElementById('previewIcone').textContent=this.value">
                    <span class="hint">Cole qualquer emoji. Sugestões rápidas:</span>
                    ...
                    <div class="fg">
                        ...
                        <div class="color-row">
                            ...
                            <input type="text" id="txt_cor_fundo" name="cor_fundo" class="fc"
                                   value="<?= hce($v['cor_fundo']) ?>" maxlength="7" placeholder="#25d366"
                    ...
                    <div class="fg">
                        ...
                        <div class="color-row">
                            ...
                            <input type="text" id="txt_cor_icone" name="cor_icone" class="fc"
                                   value="<?= hce($v['cor_icone']) ?>" maxlength="7" placeholder="#ffffff"
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-dark">💾 Salvar Ícone</button>
