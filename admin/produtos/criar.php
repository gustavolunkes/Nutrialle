...
// Gera SKU automático (PRD-XXXXXX)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!$nome) { $error = 'O nome é obrigatório.'; }
        ...
        if ($chk->fetch()) { $error = 'Este SKU já está em uso.'; }
            ...
            try {
                $db->prepare("INSERT INTO produtos (sku,nome,descricao_curta,descricao,preco,preco_promocional,imagem_principal,meta_description,meta_keywords,destaque,ativo)
                              VALUES (?,?,?,?,?,?,?,?,?,?,?)")
    ...
    <div class="card">
        ...
        <div class="form-grid">
            <div class="fg">
                ...
                <span class="hint">Código único do produto. Será usado na URL: /produto/<strong id="sku-url-prev"><?= htmlspecialchars($sku ?: $sku_sugerido) ?></strong></span>
            </div>
            <div class="fg">
            ...
            <div class="fg full">
                ...
                <input type="text" name="descricao_curta" class="fc" value="<?= htmlspecialchars($descricao_curta) ?>" placeholder="Resumo exibido na listagem de produtos (máx. 120 chars)" maxlength="120">
...
function toUpper(s){ return s.toUpperCase().replace(/[^A-Z0-9-]/g,''); }
