...
$page_title     = 'Nova Categoria de Produto';
$current_module = 'produtos';
$current_page   = 'cat-produtos-lista';
$error = '';
$db = getDB();

$nome = $slug = $descricao = $meta_description = $meta_keywords = $imagem = '';
$ordem = 0; $ativo = 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!$slug) { $error = 'O slug é obrigatório.'; }
        ...
        try {
            $chk = $db->prepare("SELECT id FROM categorias_produtos WHERE slug = ?");
            $chk->execute([$slug]);
            if ($chk->fetch()) {
                $error = 'Este slug já está em uso.';
            } else {
                $db->prepare("INSERT INTO categorias_produtos (nome,slug,descricao,imagem,meta_description,meta_keywords,ordem,ativo) VALUES (?,?,?,?,?,?,?,?)")
                   ->execute([$nome, $slug, $descricao, $imagem ?: null, $meta_description, $meta_keywords, $ordem, $ativo]);
                $_SESSION['success'] = "Categoria <strong>$nome</strong> criada com sucesso!";
...
.info-box{background:#eff6ff;color:#1e40af;border-left:4px solid #3b82f6;padding:12px 16px;border-radius:8px;font-size:13px;margin-bottom:18px}
...
<div class="pg-wrap">
    ...
    <div class="info-box">
        💡 <strong>Como funciona:</strong> Após criar a categoria, ela ficará disponível em <strong>/produtos/[slug]</strong>. Basta colocar esse link no menu de navegação.
    </div>
    ...
    <div class="card">
        ...
        <form method="POST">
            <input type="hidden" name="imagem" id="img-val" value="<?= htmlspecialchars($imagem) ?>">
            <div class="form-grid">
                <div class="fg">
                    <label>Nome <span class="req">*</span></label>
                    <input type="text" name="nome" id="inp-nome" class="fc" value="<?= htmlspecialchars($nome) ?>" required placeholder="Ex: Telefones">
                </div>
                <div class="fg">
                    <label>Slug (URL) <span class="req">*</span></label>
                    <input type="text" name="slug" id="inp-slug" class="fc" value="<?= htmlspecialchars($slug) ?>" required placeholder="telefones">
                    <div class="slug-preview">🔗 /produtos/<b id="slug-val"><?= htmlspecialchars($slug) ?: '...' ?></b></div>
                    <span class="hint">URL da página de listagem desta categoria</span>
                </div>
                <div class="fg full">
                    <label>Descrição</label>
                    <textarea name="descricao" class="fc" placeholder="Descrição exibida no topo da listagem de produtos..."><?= htmlspecialchars($descricao) ?></textarea>
                </div>
                <div class="fg">
                    <label>Ordem de exibição</label>
                    <input type="number" name="ordem" class="fc" min="0" value="<?= $ordem ?>">
                    <span class="hint">Menor número aparece primeiro</span>
                </div>
                <div class="fg">
                    <label>Meta Description <small style="font-weight:400;color:#9ca3af">(SEO)</small></label>
                    <input type="text" name="meta_description" class="fc" value="<?= htmlspecialchars($meta_description) ?>" placeholder="Para o Google (máx. 160 chars)" maxlength="160">
                </div>
                <div class="fg full">
                    <label>Meta Keywords <small style="font-weight:400;color:#9ca3af">(SEO)</small></label>
...
document.getElementById('inp-nome').addEventListener('input', function(){
    const s = toSlug(this.value);
    document.getElementById('inp-slug').value = s;
    document.getElementById('slug-val').textContent = s || '...';
});
