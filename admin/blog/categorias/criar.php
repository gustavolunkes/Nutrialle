...
$page_title     = 'Nova Categoria do Blog';
$current_module = 'blog';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!$slug) { $error = 'O slug é obrigatório.'; }
        ...
        try {
            $chk = $db->prepare("SELECT id FROM blog_categorias WHERE slug = ?");
            $chk->execute([$slug]);
            if ($chk->fetch()) {
                $error = 'Este slug já está em uso.';
            } else {
                $db->prepare("INSERT INTO blog_categorias (nome, slug, cor, ativo) VALUES (?,?,?,?)")
                   ->execute([$nome, $slug, $cor, $ativo]);
...
<div class="pg-wrap">
    <div class="pg-header">
        ...
        <h2>🏷️ Nova Categoria do Blog</h2>
    </div>
        ...
        <form method="POST">
            <div class="form-grid">
                <div class="fg">
                    ...
                    <input type="text" name="nome" id="inp-nome" class="fc" value="<?= htmlspecialchars($nome) ?>" required placeholder="Ex: Nutrição Animal">
                </div>
                <div class="fg">
                    <label>Slug (URL) <span class="req">*</span></label>
                    <input type="text" name="slug" id="inp-slug" class="fc" value="<?= htmlspecialchars($slug) ?>" required placeholder="nutricao-animal">
                    <div class="slug-preview">🔗 blog/<b id="slug-val"><?= $slug ?: '...' ?></b></div>
                </div>
                <div class="fg full">
                    <label>Cor da Categoria</label>
                    <div class="color-row">
                        <input type="color" name="cor" id="inp-cor" class="fc" value="<?= htmlspecialchars($cor) ?>" style="width:100px">
                        <div class="color-preview" id="color-preview" style="background:<?= htmlspecialchars($cor) ?>"></div>
